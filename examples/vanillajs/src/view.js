...
export default function View(template) {
	...
	this._clearCompletedButton= function(completedCount, visible) {
		this.$clearCompleted.innerHTML = this.template.clearCompletedButton(completedCount);
		this.$clearCompleted.style.display = visible ? 'block' : 'none';
	}

	this._editItemDone= function(id, title) {
		let listItem = qs(`[data-id="${id}"]`);
	...
	this._bindItemEditDone= function(handler) {
		let self = this;
	...
	this._bindItemEditCancel= function(handler) {
		let self = this;
